...
쇼핑몰 서비스의 프론트엔드 프로젝트입니다.

상품 조회, 상품 상세, 장바구니, 체크아웃, 주문, 마이페이지, QnA, 공지/FAQ, 관리자 상품·주문 관리 화면을 제공합니다.  
API 명세를 기준으로 인증 상태, 요청 구조, 응답 처리, 장바구니, 주문 흐름을 일관되게 연결하는 데 중점을 두었습니다.
...
이를 통해 페이지 코드는 화면 상태와 렌더링에 집중하고, 인증/응답/에러 처리 같은 공통 로직은 API 계층에서 일관되게 처리할 수 있도록 했습니다.
