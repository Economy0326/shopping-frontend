...
모든 요청은 인증 쿠키 사용을 위해 withCredentials: true 기준으로 동작
...
일반 API 요청 중 401 발생 시
→ /auth/refresh 자동 재시도 ❌

앱 최초 진입 시 accessToken이 없을 때만
→ /auth/refresh 1회 시도

refresh 실패 시
→ 비로그인 상태 유지 + 로그인 모달 표시
...
body === undefined이면 data 필드 자체를 보내지 않음
