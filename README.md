...
쇼핑몰에서 상품을 탐색하고 장바구니에 담은 뒤, 주문까지 이어지는 흐름을 구현한 프론트엔드 프로젝트입니다.

단순히 상품 목록과 주문 화면만 만드는 것보다, 주문 이후에 사용자가 주문을 조회하고 취소나 반품을 요청하는 흐름까지 연결하는 데 집중했습니다.

회원 주문과 비회원 주문을 모두 다뤘고, 관리자가 주문 상태를 확인하고 입금 확인, 배송 처리, 반품 승인/거절을 할 수 있는 화면도 구현했습니다.
...
백엔드는 별도 `shopping-backend` 저장소의 NestJS API와 연동했습니다.

외부 PG 결제와 택배사 배송 API는 구현 범위에서 제외했고, 무통장 입금과 관리자 배송 처리 방식으로 주문 상태 흐름을 구성했습니다.
