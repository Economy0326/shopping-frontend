...
export default function PasswordChangePage() {
  ...
  const token = sp.get("token");         // 있으면 재설정(확정) 모드
  ...
  return (
    <div>
      ...
      <main>
        <form onSubmit={onSubmit} className="rounded-2xl bg-white p-4 md:p-5 shadow-sm space-y-4">
          {!isReset && (
          ...
          <div className="pt-2 flex gap-2">
            <button
              ...
              className={`rounded-xl px-4 py-2 text-sm text-white ${
                disabled || saving ? "bg-gray-300 cursor-not-allowed" : "bg-black hover:opacity-90"
              }`}
            >
              {saving ? (isReset ? "재설정 중…" : "변경 중…") : (isReset ? "재설정하기" : "변경하기")}
            </button>

            {!isReset && (
              <Link to="/mypage" className="rounded-xl px-4 py-2 text-sm border hover:bg-gray-50">
                취소
              </Link>
            )}
          </div>
          ...
          {done && (
            <p className="text-sm text-emerald-600">
              {isReset ? "비밀번호가 재설정되었습니다. 이제 로그인해 주세요." : "비밀번호가 변경되었습니다."}
            </p>
          )}
        </form>
      </main>
