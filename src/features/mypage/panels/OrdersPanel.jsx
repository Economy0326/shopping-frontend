    ...
    <article className="w-full rounded-2xl border bg-white shadow-sm p-4 md:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
        ...
        <div className="flex flex-col justify-center gap-1 min-w-0">
          ...
          <div className="text-sm font-semibold">{formatWon(order.totalAmount)}</div>
        </div>
        ...
        <div className="sm:ml-auto w-full sm:w-56 flex items-center justify-between sm:flex-col sm:items-end sm:justify-between mt-1 sm:mt-0">
          <div className="text-left sm:text-right text-xs md:text-sm text-gray-500 leading-5 w-full sm:w-auto space-y-0.5">
            <div className="whitespace-nowrap truncate" title={order.orderDate}>
              주문 일자: {order.orderDate}
            </div>
            <div className="whitespace-nowrap truncate font-mono" title={order.orderNo}>
              주문 번호: {order.orderNo}
            </div>
          </div>
...
export default function OrdersPanel() {
  ...
  const load = async (page = 1) => {
    try {
      setLoading(true);
      const res = await OrdersAPI.list({ page, size: meta.size || 10, sort: "orderDate", order: "desc" });
      const rows = res?.data || [];
  ...
  const onConfirm = async (id) => {
    try {
      ...
      setList((prev) =>
        prev.map((o) =>
          o.orderId === id ? { ...o, status: "CONFIRMED", actions: (o.actions || []).filter(a => a !== "confirm") } : o
        )
      );
    } catch (e) {
      ...
      <div ref={listRef} className="min-h-[60vh] max-h-[80vh] overflow-y-auto pr-1">
        ...
        {loading && list.length === 0 ? (
          ...
          <div className="flex flex-col gap-4">
            {list.map((o) => (
              <OrderCard key={o.orderId} order={o} onConfirm={onConfirm} />
            ))}
          </div>
        )}
        {meta.hasNext && (
