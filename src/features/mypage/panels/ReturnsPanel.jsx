...
function ReturnCard({ ret }) {
  const helper =
    ret.status === "APPROVED"
      ? "반품이 승인되었습니다. 환불 처리 대기 중입니다."
      : ret.status === "REJECTED"
      ? "반품이 거절되었습니다. 주문 상세에서 상태를 확인하세요."
      : ret.status === "REFUNDED"
      ? "환불이 완료되었습니다."
      : "반품이 접수되었습니다. 관리자 확인 후 처리됩니다.";
    ...
    <article className="w-full rounded-2xl border bg-white shadow-sm p-4 md:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <div className="font-mono text-sm break-all">{ret.orderId}</div>

          <div className="mt-2 text-sm text-gray-500">반품ID</div>
          <div className="font-mono text-sm break-all">{ret.id}</div>
          ...
          {ret.reason && (
            <div className="mt-1 text-xs text-gray-600">사유: {ret.reason}</div>
          )}

          {ret.memo && (
            <div className="mt-1 text-xs text-gray-600">메모: {ret.memo}</div>
          )}
...
export default function ReturnsPanel() {
  ...
  const load = async (page = 1) => {
    try {
      setLoading(true);
      const res = await request(RETURNS.ROOT, {
        params: { page, size: meta.size || 10 },
      });
      const rows = res?.data ?? [];
  ...
  return (
    <PanelShell title="반품 내역">
      <div
        ref={listRef}
        className="min-h-[60vh] md:max-h-[80vh] md:overflow-y-auto pr-1"
      >
