...
export default function CheckoutPage() {
  ...
  const canSubmit =
    cart.length > 0 &&
    !!form.name && !!form.phone && !!form.address1 && !!form.depositor &&
    !!agree;
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      receiver: {
        ...
        address: { zipcode: form.zipcode, address1: form.address1, address2: form.address2 },
        memo: form.memo,
      },
    ...
    try {
    // createOrder가 지금은 동기지만, 나중에 API 모드로 바뀌면 비동기가 됨.
    // 둘 다 대응하려고 Promise.resolve로 감싼다.
    const created = await Promise.resolve(createOrder(payload));
    const newId = created?.id || created; // {id} 또는 문자열 어느쪽이든 커버
    cleanCart();
    ...
    <main className="max-w-3xl mx-auto p-6 grid gap-6">
      ...
      <form onSubmit={onSubmit} className="border rounded p-4 grid gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <input className="border p-2 rounded" placeholder="수령인 *"
            value={form.name} onChange={(e)=>setForm({...form, name:e.target.value})} />
          <input className="border p-2 rounded" placeholder="연락처 *"
            value={form.phone} onChange={(e)=>setForm({...form, phone:e.target.value})} />
          <input className="border p-2 rounded col-span-2" placeholder="이메일(선택)"
            value={form.email} onChange={(e)=>setForm({...form, email:e.target.value})} />
          <input className="border p-2 rounded" placeholder="우편번호"
            value={form.zipcode} onChange={(e)=>setForm({...form, zipcode:e.target.value})} />
          <input className="border p-2 rounded" placeholder="주소 *"
            value={form.address1} onChange={(e)=>setForm({...form, address1:e.target.value})} />
          <input className="border p-2 rounded col-span-2" placeholder="상세주소"
            value={form.address2} onChange={(e)=>setForm({...form, address2:e.target.value})} />
          <input className="border p-2 rounded col-span-2" placeholder="배송 메모(선택)"
            value={form.memo} onChange={(e)=>setForm({...form, memo:e.target.value})} />
        </div>

        <h2 className="font-bold mt-4">입금자명</h2>
        <input className="border p-2 rounded" placeholder="입금자명 *"
          value={form.depositor} onChange={(e)=>setForm({...form, depositor:e.target.value})} />
        ...
        <button
          type="submit"
          className={`mt-3 h-12 bg-red-500 text-white font-bold rounded hover:opacity-90 ${
