...
export default function MyPageLayout({ isLoggedIn, username, onLogout}) {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex">
        {/* 사이드바 */}
        <aside className="bg-red-500 w-1/4 lg:w-1/6 sticky top-16 h-[calc(100vh-4rem)] overflow-auto p-3">
          <div className="space-y-5">
            {/* 주문 관련 */}
            <div className="space-y-2">
              <div className="text-xl text-white font-bold">나의 주문</div>
              <button className={itemCls("orders")} onClick={() => setTab("orders")}>전체 주문내역</button>
              <button className={itemCls("cancellations")} onClick={() => setTab("cancellations")}>결제 취소 내역</button>
              <button className={itemCls("returns")} onClick={() => setTab("returns")}>반품 내역</button>
            </div>

            {/* 내 정보 수정 */}
            <div className="space-y-2">
              <div className="text-xl text-white font-bold">내 정보</div>
              <button className={itemCls("profile")} onClick={() => setTab("profile")}>내 정보 수정</button>
            </div>
          </div>
        </aside>

        {/* 본 내용 */}
        <section className="flex-1 p-4 min-h-[60vh]">
